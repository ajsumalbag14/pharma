...
if(isset($_POST['userID']) && isset($_POST['doctorID']) && isset($_FILES['doctorSignature']) && isset($_POST['locationLat']) && isset($_POST['locationLong'])) {
    ...
    $response = json_decode($doctorsVisit->addDoctorsVisit($doctorsVisitParams)); // returns doctorsVisitID <-- the last insert id
    ...
    if($response->result == 1){ //success
        ...
        if(isset($_POST['salesOrders'])) //process if there are records
        {
            ...
            $salesOrder = json_decode($_POST['salesOrders']);

            //echo $salesOrder[0]->productCode;

            //exit;

            $totalNetPrice = 0; //this holds the overall total of the orders

            //echo "count: " .count($salesOrder);
			...
			//06/21/2018 - remove the validation of available balance temporarily
			//because the DOCTOR_BALANCE table will not be used for the meantime. 
			//they will allow all doctors to purchase accordingly
			//this field will be ready for the succeeding phases
			/*
			...
			*/
			...
			//this field will be ready for the succeeding phases
			/*
            //deduct the total net amount to the doctor's balance
			...
			*/
            ...
            echo json_encode([
                "status"=> http_response_code(),
                "message"=> "Transaction successful. File upload completed; Doctor Visit completed; sales order completed"
            ]); 
        }
        else {
            $rest->setHttpHeaders("application/json", 200);
